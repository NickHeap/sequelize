...
const QueryGenerator = {
  ...
  versionQuery() {
    return 'SELECT "_DbStatus-DbVers" from PUB."_DbStatus"';
  },

  createTableQuery(tableName, attributes, options) {
    options = _.extend({}, options || {});

    //OpenEdgeDialect 9.0 does not support CREATE TABLE IF NOT EXISTS
    const databaseVersion = _.get(this, 'sequelize.options.databaseVersion', 0);
    const attrStr = [];
    let comments = '';
    ...
    return `CREATE TABLE ${values.table} (${values.attributes})${values.comments}`;
  },
  ...
  showTablesQuery() {
    return 'SELECT "_file-name" FROM PUB."_file" WHERE "_file"."_file-num" > 0 AND "_file"."_file-num" < 32000';
  },
  ...
  handleSequelizeMethod(smth, tableName, factory, options, prepend) {
    if (smth instanceof Utils.Json) {
      ...
      if (smth.conditions) {
        ...
        // Allow specifying conditions using the openedge json syntax
  ...
  showConstraintsQuery(tableName) {
    //OpenEdgeDialect converts camelCased alias to lowercase unless quoted
    return [
  ...
  quoteIdentifier(identifier, force) {
    ...
    if (!force && this.options && this.options.quoteIdentifiers === false && identifier.indexOf('.') === -1 && identifier.indexOf('->') === -1) { // default is `true`
      // In OpenEdgeDialect, if tables or attributes are created double-quoted,
  ...
  setAutocommitQuery(value, options) {
    ...
    // OpenEdgeDialect does not support setting AUTOCOMMIT = OFF as of 9.4.0
