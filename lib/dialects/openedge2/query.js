...
class Query extends AbstractQuery {
  ...
  run(sql, parameters) {
    ...
    if (this.sql === "SELECT 1+1 AS result") {
      this.sequelize.log('Rewriting authenticate query (' + (this.client.uuid || 'default') + '): ' + this.sql, this.options);
      this.sql = this.sequelize.getQueryInterface().QueryGenerator.versionQuery();
    }
    ...
    return query.catch(err => {
      ...
      .then(queryResult => {
        ...
        if (this.isShowIndexesQuery()) {
          ...
          let result = rows;
          // OpenEdgeDialect will treat tables as case-insensitive, so fix the case
          ...
          return rows[0]['_DbStatus-DbVers'];
  ...
  formatError(err) {
    ...
    const code = err.state || err.sqlState;
    const errMessage = err.message || err.messagePrimary;
    const errDetail = err.stack || err.messageDetail;
